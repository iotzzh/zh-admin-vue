...
const registSocket = () => {
  ...
  setInterval(() => { if (!ws.isConnection) ws.reconnection(); }, 3000);
};
...
const displayNotification = async (title:any, options:any) => {
...
const registWorker = async () => {
  ...
  registration.active.postMessage('Hi service worker'); // post message to service workerjs
