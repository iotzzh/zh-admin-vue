<template>
  <div class="zh-tree" v-loading="loadingTree">
    <div class="top-box" v-if="tData && tData.length > 0">
      ...
      <el-button class="add-button" v-if="config.treeConfig.hasRootAdd" type="success"
        @click="openModal(0)">新增</el-button>
    </div>

    <el-scrollbar class="scrollbar">
      <el-tree v-if="tData && tData.length > 0" ref="refZHTree" class="tree" :data="tData"
        :props="config.treeConfig.defaultProps" :check-strictly="config.treeConfig.checkStrictly"
        :show-checkbox="config.treeConfig.showCheckbox" :indent="0" default-expand-all :filter-node-method="filterNode"
        @node-click="onNodeClick">
        <template #default="{ node, data }">
          <span class="custom-tree-node">
            <el-tooltip popper-class="custom-tree-node-tooltip" effect="light" :disabled="!node.data[config.treeConfig.defaultProps.label] ||
    node.data[config.treeConfig.defaultProps.label].length <= labelDisplayMaxLength
    " :content="getTooltipHTMLContent(node.data[config.treeConfig.defaultProps.label])" placement="top"
              style="width: 100%" raw-content>
              <span>{{
    getTooltipOriginContent(node.data[config.treeConfig.defaultProps.label] || '')
            ...
            <span class="actions">
              ...
              <el-icon v-if="config.treeConfig.hasDelete" class="icon" color="#FF0000"
    ...
    <ZHFormModal v-if="config.formModalConfig" :model-value="formModel" :converted-model="convertedFormModel"
      :form-config="config.formModalConfig.formConfig" :modal-config="modal" @cancel="() => (modal.show = false)"
