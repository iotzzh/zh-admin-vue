  ...
  <div class="zh-tree" v-loading="loadingTree">
    ...
    <el-scrollbar class="scrollbar">
          ...
          <span class="custom-tree-node">
            <el-tooltip
              ...
              :disabled="
                ...
                node.data[config.treeConfig.defaultProps.label].length <=
                  labelDisplayMaxLength
              "
              :content="
      ...
      <div v-else class="empty">
        <el-icon
          v-if="config.treeConfig.hasEmptyAdd"
          :size="40"
          @click="openModal(0)"
        >
          <Plus />
...
onMounted(async () => {
  if (
    config.value.treeConfig.initialData === undefined ||
    config.value.treeConfig.initialData
  )
    getTreeData();
});
...
const getTooltipHTMLContent = (content: string) => {
  return (
    '<div style="width: 300px; word-break: break-all;">' + content + "</div>"
  );
};
...
const openModal = (type: number, e?: any, node?: any, data?: any) => {
  e && e.stopPropagation();
  const fields = config.value.formModalConfig?.formConfig
    ?.fields as TZHFromField[];
  if (type === 0) {
