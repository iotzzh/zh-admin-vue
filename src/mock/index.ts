...
const modulesFiles: any = import.meta.glob('./_modules/*.ts', { eager: true });
...
export function setupMock() {
  mocks.forEach((x: TMockParams) => {
    Mock.mock(new RegExp(x.url), x.type || 'get', (conditions: any) =>
      Mock.mock(x.response(conditions)),
    );
  });
}
