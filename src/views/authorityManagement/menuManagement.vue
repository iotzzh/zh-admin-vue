...
<template>
  ...
  >
    <template v-slot:zh-table-form-test>
      <el-input
        placeholder="请输入自定义搜索"
        v-model="formSettings.customModel!.test"
      ></el-input>
    </template>
  </Table>
</template>
...
const tableSettings = reactive({
  ...
  modal: {
    ...
    onAfterSubmit: async (params: any) => {
      refTable.value &&
        refTable.value.reloadTableTreeChild(params.conditions.parentId);
    },
  },
...
const opened = async (params: any) => {
  if (params.modal.type === "add") {
    ...
    const modelWithoutChild = JSON.parse(
      JSON.stringify(params.openEditModalData)
    );
